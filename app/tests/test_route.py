...
class Testroute:
    def test_add_route(self, couchbase_client, route_api, route_collection, helpers):
        ...
        response = client.post(url=f"{route_api}/{document_id}", json=route_data)
    ...
    def test_add_route_without_required_fields(
        ...
        assert validation_error["loc"] == ["body", "airline"]
        assert validation_error["msg"] == "Field required"

        validation_error = response_data["detail"][1]
        assert validation_error["type"] == "missing"
        assert validation_error["loc"] == ["body", "sourceairport"]
        assert validation_error["msg"] == "Field required"
    ...
    def test_read_route(self, couchbase_client, route_api, route_collection, helpers):
        ...
        response = client.get(url=f"{route_api}/{document_id}")
    ...
    def test_update_route(self, couchbase_client, route_api, route_collection, helpers):
        ...
        response = client.put(url=f"{route_api}/{document_id}", json=updated_route_data)
        assert response.status_code == 200
    ...
    def test_delete_route(self, couchbase_client, route_api, route_collection, helpers):
        ...
        response = client.delete(url=f"{route_api}/{document_id}")
