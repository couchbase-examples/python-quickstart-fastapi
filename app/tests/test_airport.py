...
class TestAirport:
    def test_add_airport(
        ...
        response = client.post(url=f"{airport_api}/{document_id}", json=airport_data)
    ...
    def test_add_airport_without_required_fields(
        ...
        assert validation_error["loc"] == ["body", "airportname"]
        assert validation_error["msg"] == "Field required"

        validation_error = response_data["detail"][1]
        assert validation_error["type"] == "missing"
        assert validation_error["loc"] == ["body", "country"]
        assert validation_error["msg"] == "Field required"
    ...
    def test_read_airport(
        ...
        response = client.get(url=f"{airport_api}/{document_id}")
    ...
    def test_delete_airport(
        ...
        response = client.delete(url=f"{airport_api}/{document_id}")
    ...
    def test_list_airports(self, airport_api):
        ...
        response = client.get(url=f"{airport_api}/list")
        assert response.status_code == 200
        response_data = response.json()
        # Default page size
    ...
    def test_list_airports_in_country(self, airport_api):
        ...
        country = "France"
        response = client.get(url=f"{airport_api}/list?country={country}")
        assert response.status_code == 200
        response_data = response.json()
    ...
    def test_list_airports_in_invalid_country(self, airport_api):
        ...
        response = client.get(url=f"{airport_api}/list?country=invalid")
        assert response.status_code == 200
        response_data = response.json()
    ...
    def test_direct_connections(self, couchbase_client, airport_api):
        ...
        response = client.get(url=f"{airport_api}/direct-connections?airport={airport}")
        assert response.status_code == 200
